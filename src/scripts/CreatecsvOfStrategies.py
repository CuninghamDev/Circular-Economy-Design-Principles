...
headers = ["Strategy Name","Short Name","Details","Life Cycle Stage","Evaluation Statement","Conventional Approach Replaced","Evaluation Points"]
header_key_lookup = dict()
header_key_lookup["Strategy Name"] = "actor"
header_key_lookup["Short Name"] = "short"
header_key_lookup["Details"] = "details"
header_key_lookup["Life Cycle Stage"] = "category"
header_key_lookup["Evaluation Statement"] = "eval"
header_key_lookup["Conventional Approach Replaced"] = "conventional"
header_key_lookup["Evaluation Points"] = "evalPoints"
...
header_row.append("Secondary Life Cycle Stages")
data_to_csv.append(header_row)
...
for actor in actors:
    ...
    for header in headers:
        try:
            row.append(actor[header_key_lookup[header]])
        except:
            row.append('')
    additionalStages = ""
    if len(actor["buttons"]) > 0:
        # print('trying to match buttons')
        matchedCount = 0
        for button in actor["buttons"]:
            for b in buttons:
                if button == b["title"] and matchedCount > 0:
                    additionalStages += ", " + b["stage"]
                    matchedCount += 1
                elif button == b['title']:
                    additionalStages += b['stage']
                    matchedCount += 1
    row.append(additionalStages)
...
import csv
filepath = "C:\\Users\\dschipper\\Documents\\Circular-Economy-Design-Principles\\src\\ce_strategies.csv"
